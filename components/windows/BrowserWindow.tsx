            ...
            <div className="flex items-center gap-1.5">
                <div className="flex gap-0.5">
                    <button onClick={goBack} disabled={histIdx <= 0}
                        className="w-7 h-7 rounded-lg flex items-center justify-center text-sm bg-gray-100 hover:bg-gray-200 disabled:opacity-30 transition" title="Back">←</button>
                    <button onClick={goForward} disabled={histIdx >= history.length - 1}
                        className="w-7 h-7 rounded-lg flex items-center justify-center text-sm bg-gray-100 hover:bg-gray-200 disabled:opacity-30 transition" title="Forward">→</button>
                    <button onClick={reload}
                        className="w-7 h-7 rounded-lg flex items-center justify-center text-sm bg-gray-100 hover:bg-gray-200 transition" title="Reload">↺</button>
                    <button onClick={() => navigate(HOME_URL)}
                        className="w-7 h-7 rounded-lg flex items-center justify-center text-sm bg-gray-100 hover:bg-gray-200 transition" title="Home">⌂</button>
                </div>

                <div className="relative flex-1">
                    <span className={`absolute left-2.5 top-1/2 -translate-y-1/2 text-xs select-none ${isSecure ? "text-green-500" : "text-gray-400"}`}
                ...
                {blocked ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-gray-50 text-center px-8">
                        ...
                        <p className="text-xs text-gray-500 max-w-xs">
                            ...
                            being shown inside another page.
                        </p>
                        <a href={url} target="_blank" rel="noopener noreferrer"
